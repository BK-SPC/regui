...
local PluginHandler = {
    ...
    GetPluginMeta = function(self,PluginName)
        local Meta =  game:GetService("HttpService"):JSONDecode(
            Io:Read({"ThirdPartyPlugins",PluginName,"Plugin.json"})
        )
        for Key,Value in pairs(Meta.Thumb) do
            Meta.Thumb[Key] = _ReGui.Helper.Asset:Get(
                Io:GetFullPath{"ThirdPartyPlugins",PluginName,Value}
            );
        end
        return Meta
    end;
    ...
    GetPluginSettings = function(self)
        local JSONPath = "ThirdPartyPluginSettings.json"
    ...
    GetPlugins = function(self)
        local PluginsPath = Io:GetFullPath("ThirdPartyPlugins")
        local PluginList = Io:ListFiles("ThirdPartyPlugins")
        local FinalList = {}
        for _,PluginName in pairs(PluginList) do
            PluginName = string.sub(PluginName,#PluginsPath+2,#PluginName)
            if Io:IsFile{"ThirdPartyPlugins",PluginName,"Plugin.lua"} then
    ...
    WasEnabled = function(self)
        local JSONPath = "EnabledThirdPartyPlugins.json"
